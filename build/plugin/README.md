...
Then the plugin, which carries the hooks, the skill and the `/guardian` commands:
...
Use the full HTTPS URL, not the `owner/repo` shorthand — the shorthand resolves to SSH and
fails without keys on the machine.

What installs is `build/plugin`, a directory built to hold exactly the payload the manifest
names and nothing else. A marketplace whose source is a working directory installs all of
it — `.mcp.json` included, which Claude Code would then enable in every project of everyone
who installed it. The staged tree is what the manifest points at, so that cannot happen.
